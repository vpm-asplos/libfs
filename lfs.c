//
// Created by vpm-anonymous on 2018-01-02.
//
...
int lfs_init (void *addr, int flag, int user_alloc_size) {
    ...
    if (flag == LFS_FORMAT) {
        ...
        // printf("DEBUG: lfs_init(LFS_FORMAT), bmutex_pid: 0\n"); 
        return 0;
        ...
        bmutex_pid = p->nproc;
        printf("DEBUG: lfs_init(LFS_INIT), nproc (bmutex_pid): %d\n", p->nproc); 
        p->nproc++;
        init_user();
        nonbiased_unlock(&p->super_futex);
        return 0;
...
int  lfs_detach() {
    ...
    // printf("DEBUG: lfs_detach, nproc: %d\n", p->nproc); 
    return 0;
}
