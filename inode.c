//
// Created by vpm-anonymous on 2018-01-10.
//
...
int wdir (inode_t *ip, const char *fname, uint32_t i_number) {
    ...
    char *block = (char *) get_block_abs_addr(ip,bn); // it will allocate the block if necessary
   
    assert (ip->i_size1 % sizeof(struct libfs_dirent) == 0);

    struct libfs_dirent *direntp = (struct libfs_dirent *)((uintptr_t)block + offset);
